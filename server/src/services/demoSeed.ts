import { PrismaClient, Product, Customer } from "@prisma/client";
import bcrypt from "bcrypt";

export const DEMO_EMAIL = "demo@corepanel.com";
export const DEMO_PASSWORD = "demo1234";
export const DEMO_BUSINESS_NAME = "Demo Isletme";
...
const monthsAgo = (months: number, day: number, hour = 12) => {
  const date = new Date();
...
export async function seedDemoData(prisma: PrismaClient): Promise<DemoSeedResult> {
  ...
  const customerMap: Record<string, Customer> = Object.fromEntries(
    customers.map((customer) => [customer.name, customer])
  );
  ...
  return {
    ...
    orderCount: orderSeed.length,
    saleCount: saleSeed.length,
  };
}
